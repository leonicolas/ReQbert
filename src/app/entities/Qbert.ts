...
export default class Qbert extends Entity {
  ...
  private bindWinListeners() {
    const win = this.behavior<Win>(Win);
    win.onStartListeners.add(() => {
      this.state = QbertState.Won;
      this.setActiveSprite("win-jump");
    });
    win.onLowestPointListeners.add(() => this.setActiveSprite("win"));
    win.onHighestPointListeners.add(() => this.setActiveSprite("win"));
    win.onEndListeners.add(() => this.setActiveSprite("win"));
  }

  private updateSprite() {
    if (this.behavior<Die>(Die).isDying || this.state === QbertState.Won) return;
